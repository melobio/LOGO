...
class Transformer(object):
    ...
    def build(
        ...
        # Model
        self.model = keras.Model(self.inputs, self.outputs, name=self.name)
        self.built = True
...
class BERT(Transformer):
    ...
    def get_inputs(self):
        ...
        """
        x_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Token')
        s_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Segment')

        if self.custom_position_ids:
            p_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Position')
            return [x_in, s_in, p_in]
        elif self.custom_masked_sequence:
            m_in = keras.layers.Input(shape=(self.sequence_length,), name='Masked-Token')
            return [x_in, s_in, m_in]
        elif self.type_vocab_size > 0:
            t_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Token-Type')
            return [x_in, s_in, t_in]
        else:
            return [x_in, s_in]
    ...
    def apply_embeddings(self, inputs):
        ...
        if self.use_segment_ids:
            ...
            x = self.apply(inputs=[x1, x2, x3], layer=keras.layers.Add, name='Embedding-Token-Segment')
    ...
    def apply_main_layers(self, inputs, index):
        ...
        x = self.apply(
            ...
            key_size=self.attention_key_size,
            kernel_initializer=self.initializer,
            # custom_conv_layer=self.custom_conv_layer,
            name=attention_name
        )
        x = self.apply(
            inputs=x,
        ...
        x = self.apply(
            inputs=[xi, x], layer=keras.layers.Add, name='{}-Add'.format(attention_name)
        )
        x = self.apply(
            inputs=self.simplify([x, z]),
            layer=LayerNormalization,
        ...
        x = self.apply(
            inputs=[xi, x], layer=keras.layers.Add, name='{}-Add'.format(feed_forward_name)
        )
        x = self.apply(
            inputs=self.simplify([x, z]),
            layer=LayerNormalization,
...
class ALBERT_Hierarchy(BERT):
    ...
    def __init__(
        ...
        super(ALBERT_Hierarchy, self).__init__(**kwargs)
    ...
    def get_inputs(self):
        ...
        （但允许自行传入位置id，以实现一些特殊需求）
        """
        f_in = keras.layers.Input(shape=(self.sequence_length, self.feature_length), name='Input-Feature')
...
class NEZHA(BERT):
    ...
    def apply_embeddings(self, inputs):
        ...
        x = self.apply(inputs=[x, s], layer=keras.layers.Add, name='Embedding-Token-Segment')
        x = self.apply(
            inputs=self.simplify([x, z]),
            layer=LayerNormalization,
            conditional=(z is not None),
...
class T5_Encoder(T5_Base):
    ...
    def get_inputs(self):
        ...
        """
        x_in = keras.layers.Input(shape=(self.sequence_length,), name='Encoder-Input-Token')
        return x_in
...
class T5_Decoder(Transformer):
    ...
    def get_inputs(self):
        ...
        """
        c_in = keras.layers.Input(
            shape=(self.sequence_length, self.hidden_size),
            name='Input-Context'
        )
        x_in = keras.layers.Input(shape=(self.sequence_length,), name='Decoder-Input-Token')
        return [c_in, x_in]
    ...
    def apply_main_layers(self, inputs, index):
        ...
        x = self.apply(
            inputs=[xi, x], layer=keras.layers.Add, name='%s-Add' % self_attention_name
        )
        ...
        x = self.apply(
            inputs=[xi, x], layer=keras.layers.Add, name='%s-Add' % cross_attention_name
        )
...
class Multi_Inputs_BERT(Transformer):
    """构建BERT模型
    """
    ...
    def __init__(
        ...
        super(Multi_Inputs_BERT, self).__init__(**kwargs)
        self.max_position = max_position
        self.with_pool = with_pool
        self.with_nsp = with_nsp
        self.with_mlm = with_mlm
    ...
    def get_inputs(self):
        ...
        if len(self.multi_inputs) > 0:
            ...
            for ii in range(len(self.multi_inputs)):
                t_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Token-Type_{}'.format(ii))
                inputs.append(t_in)
            return inputs
        else:
            return [x_in, s_in]
            ...
            for ii in range(len(self.multi_inputs)):
                if ii + 2 < len(inputs):
                    ...
                    type = self.apply(
                        ...
                        output_dim=self.embedding_size,
                        embeddings_initializer=self.initializer,
                        name='Embedding-Segment-{}'.format(ii)
                    )
                    # if self.custom_conv_layer:
                    #     type = self.apply(
                    #         inputs=type,
                    ...
                    #         strides=1,
                    #         padding='same',
                    #         name='Embedding-Segment-{}-Conv1D'.format(ii)
                    #     )
                    embedding_inputs.append(type)
    ...
    def load_variable(self, checkpoint, name):
        """加载单个变量的函数
        """
        variable = super(Multi_Inputs_BERT, self).load_variable(checkpoint, name)
        if name in [
            'bert/embeddings/word_embeddings',
            'cls/predictions/output_bias',
        ]:
    ...
    def create_variable(self, name, value):
        ...
        if name == 'cls/seq_relationship/output_weights':
            value = value.T
        return super(Multi_Inputs_BERT, self).create_variable(name, value)
...
class Multi_Inputs_Alt_BERT(Transformer):
    """构建BERT模型
    """
    ...
    def __init__(
        ...
        super(Multi_Inputs_Alt_BERT, self).__init__(**kwargs)
        self.max_position = max_position
        self.with_pool = with_pool
        self.with_nsp = with_nsp
        self.with_mlm = with_mlm
    ...
    def get_inputs(self):
        ...
        （但允许自行传入位置id，以实现一些特殊需求）
        """
        x_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Token-ALT')
        alt_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Token-Alt-ALT')
        s_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Segment-ALT')
        ...
        #     return [x_in, s_in, t_in]
        if len(self.multi_inputs) > 0:
            inputs = [x_in, alt_in, s_in]
            for ii in range(len(self.multi_inputs)):
                t_in = keras.layers.Input(shape=(self.sequence_length,), name='Input-Token-Type_{}'.format(ii))
                inputs.append(t_in)
            return inputs
        else:
            return [x_in, alt_in, s_in]
    ...
    def apply_embeddings(self, inputs):
        ...
        if self.custom_conv_layer:
            x1 = self.apply(
                ...
                strides=1,
                padding='same',
                name='Embedding-Token-Conv1D-2-ALT'
            )
            x2 = self.apply(
                inputs=x,
                layer=keras.layers.Conv1D,
                ...
                # dilation_rate=2,
                padding='same',
                name='Embedding-Token-Conv1D-3-ALT'
            )
            x3 = self.apply(
                inputs=x,
                layer=keras.layers.Conv1D,
                ...
                strides=1,
                padding='same',
                name='Embedding-Token-Conv1D-5-ALT'
            )
        ...
        if self.custom_conv_layer:
            alt1 = self.apply(
                ...
                strides=1,
                padding='same',
                name='Embedding-Token-Alt-Conv1D-2-ALT'
            )
            alt2 = self.apply(
                inputs=alt,
                layer=keras.layers.Conv1D,
                ...
                # dilation_rate=2,
                padding='same',
                name='Embedding-Token-Alt-Conv1D-3-ALT'
            )
            alt3 = self.apply(
                inputs=alt,
                layer=keras.layers.Conv1D,
                ...
                strides=1,
                padding='same',
                name='Embedding-Token-Alt-Conv1D-5-ALT'
            )
        s = self.apply(
            inputs=s,
            layer=Embedding,
        ...
        if self.custom_conv_layer:
            s1 = self.apply(
                ...
                strides=1,
                padding='same',
                name='Embedding-Segment-Conv1D-2-ALT'
            )
            s2 = self.apply(
                inputs=s,
                layer=keras.layers.Conv1D,
                ...
                # dilation_rate=2,
                padding='same',
                name='Embedding-Segment-Conv1D-3-ALT'
            )
            s3 = self.apply(
                inputs=s,
                layer=keras.layers.Conv1D,
                ...
                strides=1,
                padding='same',
                name='Embedding-Segment-Conv1D-5-ALT'
            )

        if self.custom_conv_layer:
            embedding_inputs = [x1, x2, x3, alt, s]
        else:
            embedding_inputs = [x, alt, s]
            ...
            for ii in range(len(self.multi_inputs)):
                if ii + 2 < len(inputs):
                    ...
                    type = self.apply(
                        ...
                        output_dim=self.embedding_size,
                        embeddings_initializer=self.initializer,
                        name='Embedding-Segment-{}-ALT'.format(ii)
                    )
                    embedding_inputs.append(type)

        x = self.apply(inputs=embedding_inputs, layer=keras.layers.Add, name='Embedding-Token-Segment-ALT')
        x = self.apply(
            inputs=self.simplify([x, p]),
            layer=PositionEmbedding,
            input_dim=self.max_position,
        ...
        x = self.apply(
            ...
            hidden_activation=self.layer_norm_conds[2],
            hidden_initializer=self.initializer,
            name='Embedding-Norm-ALT'
        )
        x = self.apply(
            inputs=x,
            layer=keras.layers.Dropout,
            rate=self.dropout_rate,
            name='Embedding-Dropout-ALT'
        )
        if self.embedding_size != self.hidden_size:
            x = self.apply(
                inputs=x,
    ...
    def apply_main_layers(self, inputs, index):
        ...
        attention_name = 'Transformer-{}-MultiHeadSelfAttention-ALT'.format(index)
        feed_forward_name = 'Transformer-{}-FeedForward-ALT'.format(index)
        attention_mask = self.compute_attention_mask()
    ...
    def load_variable(self, checkpoint, name):
        """加载单个变量的函数
        """
        variable = super(Multi_Inputs_Alt_BERT, self).load_variable(checkpoint, name)
        if name in [
            'bert/embeddings/word_embeddings',
            'cls/predictions/output_bias',
        ]:
    ...
    def create_variable(self, name, value):
        ...
        if name == 'cls/seq_relationship/output_weights':
            value = value.T
        return super(Multi_Inputs_Alt_BERT, self).create_variable(name, value)
