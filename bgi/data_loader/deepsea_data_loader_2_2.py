...
def load_data_step_bitf_1000index_1000bp(train_path='train.mat', valid_path='valid.mat', test_path='test.mat',
    ...
    word_index_from = 3
    num_word_dict = get_word_dict_6gram(word_index_from)

    print("num_word_dict: ", len(num_word_dict))
    print("num_word_dict: ", num_word_dict.get(0))
    print("num_word_dict: ", num_word_dict.get(1))
    print("num_word_dict: ", num_word_dict.get(2))
    print("num_word_dict: ", num_word_dict.get(3))
    print("num_word_dict: ", num_word_dict.get(4))
    ...
    if train_path is not None:
        ...
        for ii in tqdm(range(int(data.shape[2]/slice))):
            ...
            for jj in range(slice):
                ...
                if index % slice == 0 and index > 0:
                    print("Index:{}, Gene len:{}".format(index, len(actg)))
                ...
                if break_in_10w == True:
                    ...
                    if index % slice == 0 and index > 0:
                        ...
                        save_file = train_path.replace('.mat',
                                                       '_1000index_1000bp_{}gram_all_step{}_{}.npz'.format(
        ...
        if len(x_train) > 0:
            ...
            save_file = train_path.replace('.mat',
                                           '_1000index_1000bp_{}gram_{}_all_step{}_{}.npz'.format(
    ...
    if test_path is not None:
        ...
        for ii in tqdm(range(data.shape[0])):
            ...
            actg = data[ii, :, :] #np.matmul(actg_value, data[ii, :, :])
            ...
            if break_in_10w == True:
                ...
                if index % slice == 0 and index > 0:
                    ...
                    save_file = test_path.replace('.mat',
                                                  '_1000index_1000bp_{}gram_all_step{}_{}.npz'.format(n_gram,
                                                                                                                 step,
                                                                                                                 test_counter))
                    np.savez(save_file, **save_dict)
        ...
        if len(x_test) > 0:
            ...
            save_file = test_path.replace('.mat',
                                          '_1000index_1000bp_{}gram_{}_all_step{}_{}.npz'.format(n_gram,
                                                                                                         index,
                                                                                                         step,
                                                                                                         test_counter))
            np.savez(save_file, **save_dict)
    ...
    if valid_path is not None:
        ...
        save_file = valid_path.replace('.mat', '_1000index_1000bp_{}gram_8k_all_step{}.npz'.format(n_gram, step))
        np.savez(save_file, **save_dict)
        print("Writing to", save_file)
...
if __name__ == '__main__':
    ...
    load_data_step_bitf_1000index_1000bp(
        train_path=None, valid_path=valid_path, test_path=test_path,
        dict_path=dict_path,
        step=1,
        n_gram=1,
        shuffle=False,
